            ...
            try
            {
                ...
                foreach (var skill in skills)
                {
                    ...
                    if (Settings.phaserunEnabled)
                        try
                        {
                            // DebugWindow.LogMsg($"skill.InternalName {skill.InternalName} skill.Id {skill.Id} ");
                            // DebugWindow.LogMsg($"skill.IsOnCooldown - {skill.IsOnCooldown}, skill.SkillSlotIndex {skill.SkillSlotIndex}");
                            // DebugWindow.LogMsg($"!isAttacking - {!isAttacking}, isMoving {isMoving}");
                            // if (skill.Id == 35977)
                            // {
                            //     if (!skill.IsOnCooldown)
                    ...
                    if (Settings.anyVaalEnabled)
                        try
                        {
                            bool condition = ((MonsterCheck(Settings.anyVaalTriggerRange, Settings.anyVaalMinAny,
                                    ...
                                    x.CurrVaalSouls >= x.VaalSoulsPerUse)) || skill.Id == 33800 || skill.Id == 35848);
                            DebugWindow.LogMsg($"skill.Id {skill.Id}, skill.Name {skill.Name}, condition {condition}");
                            if (Settings.debugMode)
                            {
                            ...
                            if (SkillInfo.ManageCooldown(SkillInfo.vaalSkill, skill))
                                if ((MonsterCheck(Settings.anyVaalTriggerRange, Settings.anyVaalMinAny,
                                    ...
                                    x.VaalSkillDisplayName.Replace(" ", "") == skill.Name ||
                                    x.VaalSkillDisplayName.Replace("Vaal", "").Replace(" ", "") == skill.Name) &&
                                    x.CurrVaalSouls >= x.VaalSoulsPerUse)) || skill.Id == 33800 || skill.Id == 35848) // 33800 is vall molten shell and 35848 is vaal haste
                                {
